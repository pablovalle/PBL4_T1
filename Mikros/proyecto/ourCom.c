...
void jasoBufferretik(USART_TypeDef* usart, uint8_t *pMsg, uint32_t maxBytes){
	int i;
...
void USART_idatzi(USART_TypeDef* usart, uint8_t *msg, uint32_t maxBytes){
	...
	for (i = 0; i < maxBytes; i++){
		//TXE flag-a 0-ra dagoen artean
		while (!(usart->SR & USART_FLAG_TXE));		
		usart->DR = msg[i] & 0xFF;
		USART_Delay(300);
	} 
	//TXE=1 denean (bidaltzeko byte-rik ez dagoenean) TC=1 jarriko da
	//Itxaron aldaketa gertatu arte
